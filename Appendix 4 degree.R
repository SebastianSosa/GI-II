...
result = pref.attch(ids  = 10)
m = df.to.mat(result, 1, 2)
df= df.create(m)
df = met.strength(m, df = df)
NetExplorer::vis.net(df, m, 1,2)


# Binary correction ----------------------------------------
binary.correction <- function(m,obs){
  result = apply(m, 1, function(x,obs){
     sum(obs[x != 0])
  }, obs = obs)
  result = result + obs
  result
...
binary.correction2 <- function(m,obs){
  result = ANTs:::time.heterogeneity(obs)
  result[m == 0] = 0
  result = met.outstrength(result)
  result
}


###################################################
# Simulation 2 for degree

simulation2 <- function(ids =10, OBS = 1000, non.alters.prob = 0.1, Ngroups = 4, groups.size.scan = 6, ynull = F, nperm = 1000, bias = 10,  p2 = 0.2){
  # Data creation----------
  ...
  d2 = df.create(m)
  d2$outstrength = met.outstrength(m)
  d2$degree = met.outdegree(m)
  d2$eigen = met.eigen(m, sym = FALSE)
  d2 = merge(d2, nobs, by = "id", all = T)
  d2$degreecor1 <- d2$degree/binary.correction(m, d2$nobs)
  d2$degreecor2 <- d2$degree/binary.correction2(m, d2$nobs)
  d2$degreecor3 <- d2$degree/d2$nobs
  d2$degreecor1[is.nan(d2$degreecor1)] = NA
  ...
  while(test){
    ...
    sig = summary(lm(degree ~ trait.rand, data = d2))$coefficients[2,4] > 0.05
    # We want positive relationship to facilitate p-value computation
    coef.sign = summary(lm(degree ~ trait.rand, data = d2))$coefficients[2,1] > 0
    
    if(all(c(coef.sign, sig) == T)){test = FALSE}
  }
  p1 = ggplot(d2, aes(x = degreecor1, y = trait, label = id))+geom_text()
  p2 = ggplot(d2, aes(x = degreecor2, y = trait, label = id))+geom_text()
  p3 = ggplot(d2, aes(x = degreecor3, y = trait, label = id))+geom_text()
  ...
  tmp = df.create(m2)
  tmp$outstrength = met.outstrength(m2)
  tmp$degree = met.outdegree(m2,)
  tmp$eigen = met.eigen(m2, sym = FALSE)
  ...
  tmp$degreecor1.bias <- tmp$degree.bias/binary.correction(m2, tmp$nobs.bias)
  tmp$degreecor2.bias <- tmp$degree.bias/binary.correction2(m2, tmp$nobs.bias)
  tmp$degreecor3.bias <- tmp$degree.bias/tmp$nobs.bias
  tmp$degreecor1.bias[is.nan(tmp$degreecor1.bias)] = NA
  d2 = merge(d2, tmp, by = "id")
  p4 = ggplot(d2, aes(x = degreecor1.bias, y = trait, label = id))+geom_text()
  p5 = ggplot(d2, aes(x = degreecor2.bias, y = trait, label = id))+geom_text()
  p6 = ggplot(d2, aes(x = degreecor3.bias, y = trait, label = id))+geom_text()
  print(ggarrange(p1,p2,p3, p4,p5,p6, nrow = 2,ncol = 3))
 cat(all(d2$degreecor3 == d2$degreecor3.bias), "\n")

  # Node label ---------------

  r = c(coefficients(lm(degreecor3.bias ~ trait, data = d2))[2], 
        coefficients(lm(degreecor3.bias ~ trait.rand, data = d2))[2])
  
  for (b in 1:nperm) {
    r1 = coefficients(lm(degreecor3.bias ~ sample(trait), data = d2))[2]
    r2 = coefficients(lm(degreecor3.bias ~ sample(trait.rand), data = d2))[2]
    r = rbind(r, c(r1,r2))
  }
  r = as.data.frame(r)
  r[,3] = summary(lm(degreecor3.bias ~ trait, data = d2))$coefficients[2,4]
  cat(summary(lm(degreecor3.bias ~ trait, data = d2))$coefficients[2,4], "\n")
  r[,4] = summary(lm(degreecor3.bias ~ trait.rand, data = d2))$coefficients[2,4]
  rownames(r) = NULL
  colnames(r) = c("Degree.trait", "Degree.trait.rand", 
...
for (a in a:nrow(Mat)) {
  ...
  tmp =  simulation2(ids = Mat[a,1], OBS = Mat[a,2], non.alters.prob = Mat[a,3], ynull = F, nperm = 10000, bias = Mat[a,4], p2 = Mat[a,5])
